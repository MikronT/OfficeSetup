...
if not exist "%officeSetupISO%" (
  ...
  if not exist "%officeSetupISO%" if "%officeSetupURL%" NEQ "%additionalOfficeSetupURL%" (
    set officeSetupURL=%additionalOfficeSetupURL%
    goto :start
  )
  timeout /nobreak /t 1 >nul
)
